...
defineOptions({
  name: "User", // 用于指定组件的名称
  inheritAttrs: false, // 设置为 false，用于指示组件不会继承父组件的非props属性
});
import type { UploadFile } from "element-plus"; // 使其不报错 element-plus 没有导出的成员，要在ts中配置 "moduleResolution": "Node"
import {
  getUserPage,
  getUserForm,
  deleteUsers,
...
watchEffect(
  () => {
    // 树形组件的filter方法，接收一个参数 并且 指定其为 filter-node-method 属性的第一个参数
...
function handleDeptFilter(value: string, data: any) {
  // 该第一个参数value为树形组件的filter方法传入的参数searchDeptName.value
  if (!value) {
    // 没有输入查询值，则全部返回true
    return true;
  }
  return data.label.indexOf(value) !== -1; // 输入了查询值，符合查询条件的节点则返回 true
}
...
<template>
  <div class="app-container">
    <el-row :gutter="20">
      <!-- 部门栏 -->
      <el-col :lg="4" :xs="24" class="dept-col">
    ...
    >
      ...
      >
        ...
        <el-form-item label="所属部门" prop="deptId">
          <!-- deptList中的value就是点击选项后传递给select的v-model的值，相当于点击的选项的value赋值给了formData.deptId，作为部门ID -->
          <el-tree-select
