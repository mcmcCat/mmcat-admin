  ...
  nextTick(() => {
    for (const r of tagsViewStore.visitedViews) {
      if (r.path === route.path) {
        ...
        // route.fullPath可能包含了查询参数，而r.fullPath没有 ; 可能包含了动态路由参数，而r.fullPath没有。
...
function closeLeftTags() {
  tagsViewStore.delLeftViews(selectedTag.value).then((res: any) => {
    // 若在【关闭左侧】的tag中包含了当前的tag（即当前的route），则重新选中最后一个tag
    if (
...
<template>
  <div class="tags-container">
    <scroll-pane ref="scrollPaneRef" @scroll="handleScroll">
      <!-- :data-path="tag.path" 绑定自定义属性 data-path。可以在元素实例上用 el.dataset.path来获取。 -->
      <!-- :to="{ path: tag.path, query: tag.query }" 指定 <router-link> 的目标路由路径和查询参数 -->
      <!-- @click.middle 鼠标中键点击事件 -->
      <!-- @contextmenu.prevent 阻止默认右键菜单的展示，并调用 openTagMenu(tag, $event) 方法展示自定义的标签菜单。 -->
      <router-link
